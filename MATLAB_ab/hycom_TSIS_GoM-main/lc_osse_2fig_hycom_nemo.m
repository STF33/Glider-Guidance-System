...
p_NM = 1; % plot ssh NEMO
p_HC = 1; % plot ssh HYCOM

% Hindcasts:
%pthhnd1 = '/Net/gleam/abozec/HYCOM/TSIS/IASx0.03/hindcast_ugos_new/';
pthhnd1 = '/Net/gleam/abozec/HYCOM/TSIS/IASx0.03/hindcast_ugos_obs/';
pthhnd2 = '/Net/gleam/abozec/HYCOM/TSIS/IASx0.03/hindcast_extd_new/';  % extended PIES arrays
pthtopo = '/home/ddmitry/codes/HYCOM_TSIS/';
pthfig  = '/Net/kronos/ddmitry/hycom/TSIS/FIG/frames_ssh_OSSEv2/';
...
%varname = netcdf.inqVar(ncid,vid)
  ...
  cntr=cntr+1;
		foutN=sprintf('%snemo-ugos0_ssh_%4.4i.png',pthfig,cntr);
		foutH=sprintf('%stsis-%s_ssh_%4.4i.png',pthfig,expt,cntr);


% Skip if exist saved figures 
  if s_fig==2 & exist(foutN,'file') & exist(foutH,'file'), 
...
%  end
				...
				yl1=14.;
				yl2=31.;
				sub_plot_ssh2(fnb,enm,LONN,LATN,hhN,c1,c2,stt,INN,xl1,xl2,yl1,yl2);
				plot(LCN(1).xx,LCN(1).yy,'-','Color',clr1,'Linewidth',1.5);
				plot(LCH(1).xx,LCH(1).yy,'-','Color',clr2,'Linewidth',1.5);

		% Legend
				axes('Position',[0.22 0.22 0.2 0.1]);
				hold on;
				...
				set(gca,'xlim',[-0.05 0.7],'ylim',[0 0.4],...
            'xtick',[],'ytick',[]);
				...
				if s_fig>=1
%						fout=sprintf('%snemo-%s_ssh_%4.4i',pthfig,expt,cntr);
						fprintf('Saving %s\n',foutN);
						print('-dpng','-r150',foutN);
				end
...
% ==============
  if p_HC==1
				fnb=2;
				stt2=sprintf('HYCOM-TSIS SSH-v2 %s',date_str);
				
				sub_plot_ssh2(fnb,ssh,LON,LAT,hhN,c1,c2,stt2,INH,xl1,xl2,yl1,yl2);
				plot(LCN(1).xx,LCN(1).yy,'-','Color',clr1,'Linewidth',1.5);
				plot(LCH(1).xx,LCH(1).yy,'-','Color',clr2,'Linewidth',1.5);
