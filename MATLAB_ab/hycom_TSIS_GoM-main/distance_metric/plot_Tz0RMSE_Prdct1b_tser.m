% Plot T at Z0 depth 
%  RMSE statistics time series
% for forecast groups (i.e. initialized withe same hindcast)
%
% computed in calc_rmse_dTz0Prdct1b.m
%
% RMSE between control run (run=1) of HYCOM and 
% perturbed HYCOM analysis SSH fields
...
for ifc=1:Nfgr
  iFcst = IFCST(ifc);
  EXPT(iFcst).Name_short = sprintf('Prd1bFcst %2.2i',iFcst);
end
...
btx='plot_Tz0RMSE_Prdct1b_tser.m';

dsh = DSHIFT(irun);
if dsh>0
  srun = sprintf('Run=%i (+%i day)',irun,abs(dsh));
else
  srun = sprintf('Run=%i (-%i day)',irun,abs(dsh));
end
...
nfg=10;
anls_nm=sprintf('%s RMSE(m) mnth dT z=%im',srun, abs(Z0));
if f_prst==1
...
% Plot weekly bars
nfg=11;
anls_nm=sprintf('%s RMSE(m) weeks dT z=%im',srun,abs(Z0));
if f_prst==1
...
% Plot t/series
nfg=12;
anls_nm=sprintf('%s RMSE(m) days dT z=%im',srun,abs(Z0));
if f_prst==1
