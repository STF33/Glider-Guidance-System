% Nest files prepared from GLORYS 1/12
% Years 2011
%
% 
...
pthOLD = '/nexsan/people/abozec/TSIS/IASx0.03/nest/GLORYS/';
pthNEW = '/Net/kronos/ddmitry/hycom/TSIS/nest_adj_GLORYS/'; 
pthtopo = '/home/ddmitry/codes/HYCOM_TSIS/';
pthglor='/nexsan/people/abozec/TSIS/data/GLORYS/';
...
fprintf('\n  Correcting GLORYS-based nest files: adjust btrop=%3.2f, %i-%i\n\n',...
	adj_btrop,yr1,yr2);
...
for iy=yr1:yr2
  ...
  for iday=d1:d2
%  for iday=1:d2
    fina=sprintf('%sarchv.%4.4i_%3.3i_00.a',pthOLD,iy,iday);
...
iyr=2011;
iday=3;
finbOLD = sprintf('%sarchv.%4.4i_%3.3i_00.b',pthOLD,iyr,iday);
finaOLD = sprintf('%sarchv.%4.4i_%3.3i_00.a',pthOLD,iyr,iday);
LMASK = sub_lmask2(finaOLD,finbOLD);
...
% Calculate Montgomery potential from ssh
  montg1 = sub_montg1_regr2(finaOLD,finbOLD,fmat_nest);
...
%        lmask = LMASK.vvel;
        ...
        I=find(isnan(dmm));
        dmm(I)=0;
% Some bogus min values on land in Vbrtrp
        Imin=find(abs(dmm)>1.e3);
        if ~isempty(Imin)
          dmm(Imin)=0;
        end;      
%        dmm(lmask)=0;
	...
	td0=0;
        if minv<-1e10,
          fprintf('Unrealistic min val v bartrp %d\n',minv);
          keyboard
        end
% Parse string from old nest *b:
        aa2 = sub_parse_string_B(aa,nlev,td0,minv,maxv);
