...
#PBS -q standard
#
# Make sure modules are loaded same as for compiling HYCOM-tools 
# check Make logs
set echo
set time = 1
set timestamp
...
# --- N is data-set name, e.g. ec10m-reanal
#
source ${EX}/EXPT.src
...
#
# --- For winds, only Y01 and A are used.
#
# --- One year spin-up run.
#
#
@ ymx =  1
setenv A "l"  # the script works by 1 month, need to be changed to work for several months
setenv B "l"  # B is irrelavant, can ignore
setenv Y01 "120"
switch ("${B}")
case "${A}":
...
#
# --- mslprs forcing without wind forcing.
# --- Check to see if mslprs files exist, if not make them and wait.
#
...
#
# --- If the mslprs for the next segment does not exist,
# --- interpolate them to model grid while current segment is running.
#
  if (-e ./mslp/mslprs_${YXX}${B}.a && \
      -e ./mslp/mslprs_${YXX}${B}.b    ) then
#
# --- next segments mslprs already exists.
#
  else
    cd ./mslp
...
#
# --- Check to see if wind and flux files exist, if not make them and wait.
#
...
#
#     this segments glbrad already exists
#
...
#
#     this segments lwdflx already exists
#
...
cd ${S}
foreach dd(flux grad lrad mslp pcip wvel)
	cd ${dd}
	/bin/mv *_${Y01}${A}.[ab] ../DUMP/.
	cd ${S}
end
