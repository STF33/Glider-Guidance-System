% Free run from HYCOM 
% also plot contour from hindcasts HYCOM assimilating NEMO ssh
%
...
pthd    = '/Net/gleam/dmitry/hycom/TSIS/IASx0.03/output/2011_GLfreerun/'; % HYCOM freerun
pthtopo = '/home/ddmitry/codes/HYCOM_TSIS/';
pthfig  = '/Net/kronos/ddmitry/hycom/TSIS/FIG/frames_ssh_freerun/';
pthmat  = '/Net/kronos/ddmitry/hycom/TSIS/datamat/';
%if strcmp(expt,'ugosE');
%  pthfig  = '/Net/kronos/ddmitry/hycom/TSIS/FIG/frames_ssh_freerun/';
%end

btx = 'lc_osse_hycomFreeRun_nemo.m';
...
%varname = netcdf.inqVar(ncid,vid)
  ...
  cntr=cntr+1;
		foutH=sprintf('%shycomFree-nemo_ssh_%4.4i.png',pthfig,cntr);
...
% Add LC contours from the hindcast UGOS
  Ih = find(TMh==dnmb);
  xh1 = LCXY(2).XY(Ih).X;
  yh1 = LCXY(2).XY(Ih).Y;
...
%  end
    ...
    plot(xh1,yh1,'-','Color',clr3,'Linewidth',1.5);
		...
		% Legend
				...
				text(0.22, 0.3,'HYCOM-TSIS LC','Fontsize',12);
    plot([0 0.2],[0.5 0.5],'-','Color',clr3,'Linewidth',1.5);
    text(0.22, 0.5,'HYCOM Free LC','Fontsize',12);
				set(gca,'xlim',[-0.05 0.8],'ylim',[0 0.7],...
            'xtick',[],'ytick',[]);
...
% ==============
    ps1=POS(2,:);
			 stt2=sprintf('HYCOM-FreeRun SSH %s',date_str);
    clb=0;				

				sub_plot_ssh3(ps1,ssh,LON,LAT,hhN,c1,c2,stt2,INH,xl1,xl2,yl1,yl2,clb);
				plot(LCN(1).xx,LCN(1).yy,'-','Color',clr1,'Linewidth',1.5);
				plot(LCH(1).xx,LCH(1).yy,'-','Color',clr3,'Linewidth',1.5);
    plot(xh1,yh1,'-','Color',clr2,'Linewidth',1.5);
