...
%
%              contour(time,log(period),log(power))    <----  Dmitry: using log(power) does not make sense
%              plot(time,log(coi),'k')
%
%----------------------------------------------------------------------------
...
%----------------------------------------------------------------------------
%
%  With some changes by Dmitry
%
function [wave,period,scale,coi] = ...
	wavelet(Y,dt,pad,dj,s0,J1,mother,param);

%fprintf('Function: wavelet \n');

if (nargin < 8), param = []; end
if (nargin < 7), mother = []; end
if (nargin < 6), J1 = []; end
if (nargin < 5), s0 = []; end
if (nargin < 4), dj = []; end
...
if isempty(s0), s0=2*dt; end
if isempty(dj), dj = 1./4.; end
if isempty(J1), J1=fix((log(n1*dt/s0)/log(2))/dj); fprintf('J1 chosen: %i \n',J1); end
if isempty(mother), mother = 'MORLET'; end

%....construct time series to analyze, pad if necessary
x(1:n1) = Y - mean(Y);
if (pad == 1)
%	base2 = fix(log(n1)/log(2) + 0.4999);   % power of 2 nearest to N   ???
	base2 = ceil(log2(n1));   % power of 2 nearest to N
