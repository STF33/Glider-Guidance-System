    function draw_ellipse(x0,y0,U1,V1,sclv,x1,y1,Uvct,unts,COLR);
%
% Calculate and draw STDev ellipses and mean vector
% from U,V time series at location x0, y0
...
% unts - 'cm/s' or 'm/s'
% COLR = 2X3 - colors for vector and ellipse
...
%      plot([ii ii+mux],[jj jj+muy],'r');
  pp8=plot(ii,jj,'r.');
  set(pp8,'Color',col,'MarkerSize',10);
%      hold on
%  col_ell=[0.6 0.6 0.6];   % ellipse color
  col_ell=COLR(2,:);   % ellipse color
  lwde=1.2;
...
if ~isempty(x1)
  ...
  text(x1,y1+0.4*(x2-x1),ustr,'Fontsize',12);
end
