...
import numpy as np
import os
from A_functions import calculate_gridpoint, plot_formatted_ticks, plot_contour_cbar, plot_bathymetry, datetime_title, print_starttime, print_endtime, print_runtime
...
def GGS_plot_currents(config, directory, model_data, depth_average_data, qc_latitude, qc_longitude, density=2, show_route=False, show_qc=False):
    ...
    data_extent_lon = [model_data.lon.min(), model_data.lon.max()]
    data_extent_lat = [model_data.lat.min(), model_data.lat.max()]
    ...
    cbar = fig.colorbar(contourf, cax=cbar_ax, orientation='vertical', extend=extend)
...
### FUNCTION:
# @profile
def GGS_plot_threshold(config, directory, model_data, depth_average_data, qc_latitude, qc_longitude, density=2, mag1=0.0, mag2=0.2, mag3=0.3, mag4=0.4, mag5=0.5, show_route=False, show_qc=False):
    ...
    contourf = ax.contourf(longitude, latitude, mag_depth_avg, levels=levels, colors=colors, extend='both', transform=ccrs.PlateCarree(), zorder=10) # zorder = [1]
    streamplot = ax.streamplot(longitude, latitude, u_depth_avg, v_depth_avg, transform=ccrs.PlateCarree(), density=density, linewidth=0.5, color='black', zorder=10)
