...
import numpy as np
import os
from A_functions import calculate_gridpoint, plot_profile_thresholds, print_starttime, print_endtime, print_runtime
...
### FUNCTION:
# @profile
def GGSS_plot_qc(config, directory, model_qc, depth_average_data, bin_average_data, qc_latitude, qc_longitude, threshold=0.5):
    ...
    - model_qc (xarray.Dataset): Model dataset containing 'u' and 'v' data.
    ...
    (y_model_index, x_model_index), _ = calculate_gridpoint(model_qc, qc_latitude, qc_longitude)
    ...
    u_model = model_qc['u'].isel(y=y_model_index, x=x_model_index).values
    v_model = model_qc['v'].isel(y=y_model_index, x=x_model_index).values
    ...
    axes[0].scatter(u_model, model_qc['depth'], marker='x', color='black', s=100, label='Model Datapoint', alpha=1.0, zorder=3)
    ...
    axes[1].scatter(v_model, model_qc['depth'], marker='x', color='black', s=100, label='Model Datapoint', alpha=1.0, zorder=3)
