	function [gca1,az,cc1,hh1,hplt,hx] = ...
									contourf_2 (XX,YY,ZZ,cct,cmp,nsint,COLB,poscb,...
															wdth,hght,mint,fsz)   
% 
% =================================================================
...
% 	W  - white
%   K  - Black
% Recommended # of CCT not to exceed 6, white color is used only 
% to patch intervals which are out of interest (for example,
% interval around zero, -2 to 2 can be white).
% Number of CCT can be more than 8 if several (adjacnet) intervals are 
% conceived to be white. 
% Each color interval (except for WHITE) is then subdivided
% into NSINT subintervals, 1<=NSINT<=6. If NSINT=1, then
...
% ===============================================================
	...
	nn=length(cct);
	if (nn>8); 
		disp('Warning: Number of specified intervals greater than 8');
	end;
...
% ----------------------
	...
	clrs=zeros(nn,3);
	i1=find(cmp=='B');
	if (~isempty(i1)); clrs(i1,:)=[0,0,1]; end;
	i1=find(cmp=='M');
	if (~isempty(i1)); clrs(i1,:)=[.9,0,1]; end;
	i1=find(cmp=='C');
	if (~isempty(i1)); clrs(i1,:)=[0,1,1]; end;
	i1=find(cmp=='Y');
	if (~isempty(i1)); clrs(i1,:)=[1,1,0]; end;
	i1=find(cmp=='G');
	if (~isempty(i1)); clrs(i1,:)=[0,1,0]; end;
	i1=find(cmp=='R');
	if (~isempty(i1)); clrs(i1,:)=[1,0,0]; end;
	i1=find(cmp=='K');
	if (~isempty(i1)); clrs(i1,:)=[0,0,0]; end;
	iW=find(cmp=='W');
	if (~isempty(iW)); clrs(i1,:)=[1,1,1]; end;

% ---------------------------------------------------------
% Creat new arrays of data intervals (cct2) and colormap (jjs)
% with specified subintervals
% Note that for negative values, colors should change 
% from dark shadows to bright, so first I determine # of 
% negative intervals in the data
% ---------------------------------------------------------

	ineg=find(cct<0);
	mneg=length(ineg);
	
	dltC=(1-.4)/nsint; % interval in colormap
	dltD=abs(cct(2)-cct(1))/nsint;
	
	clear jjs cct2 
	
	nn2=0;  % First color for the out of the range value
	for kk=1:nn-1;
	   mm=0;
		for jj=1:nsint;
		 nn2=nn2+1;
...
% ----------------------------------------
    ...
    set(gca1,'position',[pos(1),pos(2),pos(3)-1.2*wdth,pos(4)]);
    wcolb=wdth;
    hcolb=hght*pos(4);
    xcolb=pos(1)+pos(3)-.8*wdth;
...
% ----------------------------------------
	if (poscb=='right')|(poscb=='RIGHT'); 
 		...
 		for ll=1:mint:nn,
   		...
   		text(xtxt,yxt,num2str(cct(ll)),'fontsize',fsz,...
