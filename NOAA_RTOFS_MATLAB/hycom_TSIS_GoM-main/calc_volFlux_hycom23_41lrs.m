...
% use only for archv files - U&V not collocated
HR=0;
%esim='freerun';  % free run with GOFS3.1 OB, HYCOM2.3
esim='hindcast'; % data-assimilative hindcast with HYCOM-TSIS
...
btx = 'calc_volFlux_hycom23_41lrs.m';
...
fmat = sprintf('%s%s_gofs31_VolTrt_%s_%4.4i-%4.4i.mat',...
...
%keyboard
  ...
  % Save mat file
    ...
    switch(esim)
     case('freerun'),
      pthi=sprintf('/nexsan/people/ddmitry/hycom/TSIS/IASx0.03/output_gofs3.1_freerun/%4.4i/',iyr);
			fina = sprintf('%sarchv.%i_%3.3i_%2.2i.a',pthi,iyr,iday,HR);
			finb = sprintf('%sarchv.%i_%3.3i_%2.2i.b',pthi,iyr,iday,HR);
     case('hindcast'),
      pthi=sprintf('/nexsan/people/ddmitry/hycom/TSIS/IASx0.03/output_hindcast_anlysis/%4.4i/',iyr); 
      fina = sprintf('%sarchv.%i_%3.3i_%2.2i.a',pthi,iyr,iday,HR);
      finb = sprintf('%sarchv.%i_%3.3i_%2.2i.b',pthi,iyr,iday,HR);
     case('gofs_nest');
      pthi=sprintf('/Net/kronos/ddmitry/hycom/TSIS/nest_files_gofs3.1/%4.4i/',iyr);
			fina = sprintf('%sarchm.GLBb008ias003topo_%i_%3.3i.a',pthi,iyr,iday,HR);
			finb = sprintf('%sarchm.GLBb008ias003topo_%i_%3.3i.b',pthi,iyr,iday,HR);
    end
...
tp=-1.e-6*TRP(3).TBtp; % approximate Yuc. trpt from barotropic component
