...
btx = 'anls_fcst_mhd_LCcntr.m';

iHnd = 2;  % hindcast group to analyze, 1 - hindcast#7, 2 - #8, 3 - #2
Nruns=7;  % # of runs in 1 forecast series
...
icc=0;
for ill=1:nll
  if MHD(ill).iHindcast ~= iHnd; continue; end;
  icc=icc+1;
...
for itime=1:2 % summary for 2 time period of forecasts
  ...
  dstr2=MHD(j2).Date_str;
		stl=sprintf('MHD HYCOM/NEMO LC %s/%s, Ini:%s-%s',ds1,ds2,dstr1,dstr2);
  title(stl);
end
