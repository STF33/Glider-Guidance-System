...
%  these are runs 10, 11, 12, ..., 16 - 7 f/casts
%
% Extract and save LC contours from HYCOM free runs / forecasts and
% nemo simulations 
% specify individually which run need to extract
%
%  NEMO is extracted in extr_lc_hycom_nemo.m 
%
...
itime0 = 1; % =1 - 2011, 2= 2012
ifcst1 = 10; % iFcst = hindcast # used for initial fields
ifcst2 = 16; % f/casts
% irun1=irun2=1 - keep 1 - not shifted f/casts
%irun1 = 1;  % all expts 1a (HYCOM vs NEMO) has only 1 run, irun=1
%irun2 = 1;  % iruns=2:5 - expts 1b - shifted +/- 2, 1 days - perturbed OC
...
FCST = sub_fcstPrdct_info(ifcst1);
...
% Average by months
  ...
  nfg=(itime-1)*3+imo;
  stl = sprintf('Predctb1a Fcst#%i, Z0=%im, run %2.2i: %s, Mo %i',...
               iFcst,irun,Z0,Time_name,imo);
