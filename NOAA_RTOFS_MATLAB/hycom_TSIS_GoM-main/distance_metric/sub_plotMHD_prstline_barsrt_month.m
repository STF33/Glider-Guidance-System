...
%keyboard
  ...
  for jm=1:3
    ...
    for ix0=1:length(I)
      ...
      dy=SC(ifc,jm);
      ixM = ixx;
      patch([ixx-dxx ixx-dxx ixx+dxx ixx+dxx],[0 dy dy 0],clr1,'Edgecolor','none');
    % Range:
      llw = FC1(ifc,jm);
      lup = FC2(ifc,jm);
...
% Persistence:
  ...
  for jm=1:3
    ...
    for ix0=1:length(I)
      ...
      lup = Fc2;
% Persistence:
      plot(ixM,mFc,'.','Color',clrP,'Markersize',20);
      plot([ixM-dbr ixM+dbr],[llw llw],'-','Color',clrP,'linewidth',1.5);
      plot([ixM-dbr ixM+dbr],[lup lup],'-','Color',clrP,'linewidth',1.5);
      plot([ixM ixM],[llw lup],'-','Color',clrP,'linewidth',1.5);
