% Calculate RMSE between 2 OSE hindcasts
...
for iyr = ys:ye
  pthi1=sprintf('/Net/gleam/dmitry/hycom/TSIS/IASx0.03/output/%4.4i_PIES/',iyr);
  pthi2=sprintf('/Net/gleam/dmitry/hycom/TSIS/IASx0.03/output/%4.4i_noPIES/',iyr);

  fcstname = sprintf('OSE_hindcast');
  frmseout = sprintf('%sRMSE%4.4im_OSEhindcast_%i.mat',pthout,abs(Z0),iyr);
...
% skip finished mat files
  ...
  for ii=1:nrc
    ...
    iday= YRPLT(ii,2);
    HR  = 0;
    fina = sprintf('%sarchv.%i_%3.3i_%2.2i.a',pthi2,iyr,iday,HR);
    finb = sprintf('%sarchv.%i_%3.3i_%2.2i.b',pthi2,iyr,iday,HR);
