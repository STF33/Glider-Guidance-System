...
% with persistence
%
function sub_plotMHDprst_bars_month(nfg,Nfgr,IFCST,CLR,POOL,EXPT,anls_nm);
  ...
  for ifc=1:Nfgr  % forecast groups
    ...
    for jm=1:3 % 30-day intervals
      if jm==1
        ...
        dmmp = POOL(ifc).Time(itime).pm1prs;
      elseif jm==2
        dmm  = POOL(ifc).Time(itime).pm2;
        dmmp = POOL(ifc).Time(itime).pm2prs;
      else
        dmm  = POOL(ifc).Time(itime).pm3;
        dmmp = POOL(ifc).Time(itime).pm3prs;
      end
