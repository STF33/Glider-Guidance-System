...
%irc0  = 22; % model day wrt control run that is 8 days earlier than t0 for perturb runs
%IRC = [22; 52; 82];  % day 3 wrt to day=0 of the control run,
IRC = 96;
nirc = length(IRC); 
iFcst = 14;
...
btx='plot_sshPrdct1b_LCLCEcntr.m';
...
% Plot control:
		plot(xh1,yh1,'.','Color',[0 0 0]);
  Nlcec = length(LCEN);
  for ilce = 1:Nlcec
    nee = length(LCEN(ilce).XY);
    if irc0>nee, continue; end;
    xlce = LCEN(ilce).XY(irc0).X;
    ylce = LCEN(ilce).XY(irc0).Y;
...
% Plot all LCEs
	...
	stl=sprintf('ssh, HYCOM Fcst %i  day=%3i, %2.2i/%2.2i/%4.4i',iFcst,irc,DV(3),DV(2),DV(1));
	title(stl);

	hb=colorbar('SouthOutside');
	set(hb,'Position',[0.2 0.12 0.5 0.015],...
	'Ticks',[-1:0.25:1],...
	'Fontsize',13);
	...
	for irun=irun1:irun2
		...
		y1=5-irun;
		plot([x1 x2],[y1 y1],'-','Color',clr,'Linewidth',2);
		sxx = sprintf('dt=%3i days',DT(irun));
		text(x2+0.03,y1,sxx,'Fontsize',12);
	end
	...
	bottom_text(btx,'pwd',1,'Position',[0.08 0.04 0.4 0.04]);
