...
%
%  August 2022: Changed - Average U vertical section
%  interpolated onto fixed Z depths
%
%
...
%keyboard
  ...
  %  =================
    ...
    for ks=1:NB
      ...
      UFLX(ks).ZZ=ZZ;
      UFLX(ks).ZZ_intrp=ZZf; % fixed depth Z levels
    end
...
% Interpolate onto fixed Z levels for ease of averaging & plotting
% to avoid tracking layer thicknesses etc
    for kb = 1:NB
      ZZ = UFLX(kb).ZZ;
      V  = UFLX(kb).V;
      Vi = sub_interp2z(V,ZZ,ZZf);
      UFLX(kb).Vi = Vi;
    end
...
% Average fixed depth V:
      if cc==1
        UFLX(kb).Vav=UFLX(kb).Vi;
      else
        UFLX(kb).Vav=UFLX(kb).Vav+UFLX(kb).Vi;
      end
      Vav = UFLX(kb).Vav/cc;
      VdT = Vav;
...
%      TRP(kb).TDav(cc,:)=nansum(VF,1);  % transport averaged over the total depth
      TRP(kb).VvelAv = Vav;    % mean U, l. thick is considered 
  ...
  %keyboard
      ...
      fprintf('====> Max Uav=%5.2f, Mean dP=%8.1f\n\n',...
       nanmax(nanmax(Vav)),nanmean(nanmean(sthk/cc)));
    end;
