...
def main(date_indices=None):
    ...
    for datetime_index in date_list:
        ...
        depth_average_data, bin_average_data = interp_depth_average(config, directory, rtofs_data)

        qc_latitude = '21.100'
        qc_longitude = '-86.25'
        qc_uv_profile(config, directory, rtofs_qc, depth_average_data, bin_average_data, qc_latitude, qc_longitude)

        GGS_plot_currents(config, directory, rtofs_data, depth_average_data, qc_latitude, qc_longitude, show_route=False, show_qc=False)
        GGS_plot_threshold(config, directory, rtofs_data, depth_average_data, qc_latitude, qc_longitude, mag1=0.0, mag2=0.2, mag3=0.3, mag4=0.4, mag5=0.5, show_route=False, show_qc=False)
