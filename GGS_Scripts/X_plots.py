...
import numpy as np
import os
from X_functions import calculate_gridpoint, get_6h_interval, add_formatted_ticks, calculate_cbar_ticks, add_bathymetry, get_filename_datetime
...
def GGS_plot_currents(config, directory, model_data, depth_average_data, qc_latitude, qc_longitude, density=2, show_route=False, show_qc=False):
    ...
    subtitle_text = f"[RTOFS] {dt.datetime.now(timezone.utc).strftime('%Y-%m-%d')} {get_6h_interval()} UTC"
