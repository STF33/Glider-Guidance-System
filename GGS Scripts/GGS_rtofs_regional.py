...
def GGS_config_static():
    ...
    config = {
        "glider_name": "Sentinel", #
        "max_depth": 1000.0, #
        "avg_velocity": 0.5, #
        "battery_capacity": 3000, # 3x G3
        "battery_drain": 5, #
...
def GGS_plot_main(config, waypoints, directory, RTOFS_class, u_avg, v_avg, magnitude, extent='map', show_route=True):
    ...
    title = f"{config['glider_name']} Mission Route Along Depth-Averaged Currents"
...
def main():
    ...
    RTOFS_class = RTOFS()
    RTOFS_class.rtofs_subset(config, waypoints, subset=False)

    u_avg, v_avg, magnitude = depth_avg_currents(RTOFS_class, mask=True)

    GGS_plot_main(config, waypoints, directory, RTOFS_class, u_avg, v_avg, magnitude, extent='map', show_route=True)
